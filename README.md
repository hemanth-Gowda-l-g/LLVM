# LLVM AMD Optimizer Pass

This repository contains a custom LLVM function pass plugin (`amd-opt`) that performs:
...
- `src/AMDOptimizerPass.cpp`: LLVM pass implementation
...
This script will:
1. Build the pass plugin (`build/libAMDOptimizer.so`)
2. Compile each file in `examples/` to LLVM IR
3. Run `amd-opt` on each IR file
